...
@push('footer')
    ...
    <script>
        ...
        document.addEventListener('payment-form-reloaded', function() {
            redirectPaymentButton2();

            const btnPay = document.getElementById('revisar');
            const payment_cod = document.getElementById('payment_cod');

            if(payment_cod.checked){
                btnPay.setAttribute('disabled', 'true');
            }

            payment_cod.addEventListener('change', function() {
                if (payment_cod.checked) {
                    btnPay.setAttribute('disabled', 'true');
                } else {
        ...
        function redirectPaymentButton2() {
            const url = "{{ route('button_payment', $token) }}";
            $('#buttonRedirectPay').attr('href', url);
        }
